...
```bash
flashcards$ sam build --use-container
```
...
```bash
flashcards$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
flashcards$ sam local start-api
flashcards$ curl http://localhost:3000/
```
...
```bash
flashcards$ sam logs -n HelloWorldFunction --stack-name flashcards --tail
```
...
```bash
flashcards$ pip install -r tests/requirements.txt --user
# unit test
flashcards$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
flashcards$ AWS_SAM_STACK_NAME="flashcards" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "flashcards"
```
